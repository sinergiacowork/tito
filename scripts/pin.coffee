...
module.exports = (robot) ->
  robot.respond /(pin|pinterest)( me)? (.*)/i, (msg) ->
    pin_me msg, 'http://www.pinterest.com/search/pins/?', msg.match[3], (url) ->
      msg.send url

pin_me = (msg, url, query, cb) ->
  http msg, url, query, (err, res, body) ->
    pin_url = get_pin_url(body, 'a.pinImageWrapper')
    if pin_url?
      http msg, pin_url, null, (err, res, body) ->
        cb get_pin_img(body, 'img.pinImage')
...
get_pin_url = (body, selector) ->
  ...
  'http://www.pinterest.com' + pins[idx].attribs.href
